<?php
	...
	*/
	 ...
	 * - IBAN und BIC im WooCommerce-Benutzerprofil änderbar machen
	...
	/**
	 * Include some PEAR libs
	 * if the libs are installed
	 */
	$lib = 'Validate/Finance.php';
	$paths = explode( PATH_SEPARATOR, get_include_path() );
	$found = false;
	foreach( $paths as $p )
	{

		$fullname = $p . DIRECTORY_SEPARATOR . $lib;
		if( is_file( $fullname ) )
		{
	...
	/**
	 * Include the PHP-IBAN libs
	 * if Validate/Finance could not included
	 */
	if( !class_exists( 'Validate_Finance_Iban' ) )
		require_once( plugin_dir_path( __FILE__ ) . 'php-iban/oophp-iban.php' );
		...
		class SPWC_Payment_Gateway_SEPA extends WC_Payment_Gateway
		{
			...
			public function validate_fields()
			{
				...
				if( empty( $sepaIBAN ) ){
					...
					wc_add_notice( __( 'IBAN must be filled.', 'spwcsp' ), 'error' );
					return( false );
				...
				if( empty( $sepaBIC ) ){
					...
					wc_add_notice( __( 'BIC must be filled.', 'spwcsp' ), 'error' );
					return( false );
	...
	function spwcsp_checkSepaIBAN( &$sepaIBAN, $sepaBIC = '', $empty = true )
	{
		...
		if( !empty( $sepaIBAN ) )
		{
			...
			/**
			 * If the PEAR class to validate the IBAN
			 * is loaded, we use this
			 */
			if( class_exists( 'Validate_Finance_Iban' ) )
			{

				$iban = NEW Validate_Finance_Iban();
				$iban->setIBAN( $sepaIBAN );
				if( $iban->validate() != TRUE )
					return( __( 'IBAN is not valid.', 'spwcsp' ) );


			/**
			 * Check the IBAN with the PHP-IBAN lib
			 */
			} elseif( class_exists( 'IBAN' ) )
			{
